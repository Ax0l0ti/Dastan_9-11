...
//{
    ...
    class Dastan
    {
            ...
            while (!GameOver)
            {
                ...
                do
                {
                    ...
                    if (Choice == 7 )
                    {
                        ...
                        while (!SquareIsValid)
                        {
                            StartSquareReference = GetSquareReference("containing the piece to make into Kotla");
                            SquareIsValid = CheckSquareIsValid(StartSquareReference, true);
                            if (Board[GetIndexOfSquare(StartSquareReference)].ContainsKotla())
                            {
                                SquareIsValid = false;
                            }
                        }
                        ...
                        { 
                            Board[GetIndexOfSquare(StartSquareReference)] = new Kotla(CurrentPlayer, "K");
                        }
                        else
                        {
                            Board[GetIndexOfSquare(StartSquareReference)] = new Kotla(CurrentPlayer, "k");
                    ...
                    //if choice sahm and sahm used, 
                    if (CurrentPlayer.ChoiceIsSahm(Choice) && CurrentPlayer.GetSahmStatus())
                    {
                        Choice = 10;
                    }
                }
                ...
                //Task 10 shenanigans
                // do stuff if not kotla garbage 
                if (Choice != 7)
                {


                    //Task 9 normal if not sahm or there is no sahm
                    if (!CurrentPlayer.ChoiceIsSahm(Choice) || !CurrentPlayer.GetSahmStatus() )
                    {
                        int StartSquareReference = 0;
                        while (!SquareIsValid)
                        ...
                        bool MoveLegal =
                            CurrentPlayer.CheckPlayerMove(Choice, StartSquareReference, FinishSquareReference);
                        if (MoveLegal)
                        {
                    ...
                    else
                    {
                        ...
                        while (!SquareIsValid)
                        {
                            StartSquareReference = GetSquareReference("containing the piece to shoot Sahm");
                            SquareIsValid = CheckSquareIsValid(StartSquareReference, true);
                        }
        ...
        {
            int d = CurrentPlayer.GetDirection();
            int current = StartSquareReference;
            int points = 0;
            while ((current / 10) != 0  && (current / 10) != NoOfRows + 1 )
            {
                // if current square piece not player && not null
                if (Board[GetIndexOfSquare(current)].GetPieceInSquare() != null)
                {
                    if (Board[GetIndexOfSquare(current)].GetPieceInSquare().GetBelongsTo() != CurrentPlayer )
                    {
                        if (!Board[GetIndexOfSquare(current)].ContainsKotla())
                        {
                            points += CalculatePieceCapturePoints(current);
                            Board[GetIndexOfSquare(current)].RemovePiece();
                        }
            ...
            do
            {
                switch (subChoice)
                {
                    ...
                    case 2:
                        ...
                        //if same, 0 = false 1 = true 
                        int opp = Convert.ToInt32(Equals(CurrentPlayer, Players[0]));
                        Console.WriteLine($"opp is {opp}");
                        MoveOptionQueue snapShot = CurrentPlayer.GetMoveOptionQueue();
                        CurrentPlayer.ReplaceQueue(Players[opp].GetMoveOptionQueue());
                        Players[opp].ReplaceQueue(snapShot);
                        break;
                    case 3:
                        CurrentPlayer.SwapFirstAndLast();
                        break;
                    case 4:
                        Console.WriteLine("Choose option to put at front");
                        var pos = Convert.ToInt32(Console.ReadLine());
                        CurrentPlayer.Queue.MoveItemToFront(pos-1);
                        break;
                    case 5:
    ...
    class MoveOptionQueue
    {
        ...
        {
            int rear = Queue.Count - 1;
            MoveOption Temp = Queue[rear];
            //move front to back and
            Queue.RemoveAt(rear);
            MoveItemToBack(0);
            Queue.Insert(0,Temp);
    ...
    class Player
    {
        ...
        public MoveOptionQueue Queue = new MoveOptionQueue();

        public bool GetSahmStatus()
        {
            return SahmUsed;
        }
        public void SetSahmUsed(bool Set )
        {
            SahmUsed = Set;
        }
        ...
        {
            if (Queue.GetMoveOptionInPosition(choice - 1).GetName() == "Sahm")
            {
                return true;
            }
        ...
        public void ReplaceQueue(MoveOptionQueue Replacement)
        {
...
Enter the square containing the piece to shoot Sahm (row number followed by column number): 54
